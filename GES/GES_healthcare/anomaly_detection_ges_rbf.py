"""
==============================================================
 Causal Discovery & Anomaly Detection (GES RBF - Healthcare)
==============================================================
Author: (Sagar)
"""
...
# Thesis-Verified Hyperparameters for RBF Healthcare
RBF_GAMMA = 0.001
...
def learn_causal_model(normal_csv_path: str, save_path: str):
    ...
    # Strict DAG enforcement. causal-learn encodes an edge i -> j as
    # adj_matrix[i, j] == -1 (tail at i) together with adj_matrix[j, i] == 1 (arrowhead at j).
    # Using the raw matrix also admits undirected (-1/-1) and bidirected (1/1) edges, which are
    # not causal parents; feeding those into the regression creates mutual-dependency cycles
    # where two variables are each other's predictor. Only unambiguous directed edges are kept.
    directed = np.zeros_like(adj_matrix, dtype=float)
    n_dir = 0
    for i in range(num_features):
        for j in range(num_features):
            if adj_matrix[i, j] == -1 and adj_matrix[j, i] == 1:
                directed[i, j] = 1.0
                n_dir += 1
    print(f"GES edges retained: {n_dir} directed (of {int(np.sum(adj_matrix != 0))} raw adjacency entries)")

    val_matrix[:, :, 1] = directed
    p_matrix[:, :, 1] = np.where(directed != 0, 0.0, 1.0)
...
def fit_normal_coeffs(normal_data: np.ndarray, causal_matrix: np.ndarray, alpha=RIDGE_ALPHA):
    ...
    for var in np.unique(indices[1, :]):
        ...
        rbf_map = RBFSampler(gamma=RBF_GAMMA, n_components=RBF_N_COMPONENTS, random_state=RBF_RANDOM_STATE)
...
def compute_online_errors(data: np.ndarray, fine_coeffs: dict, causal_matrix: np.ndarray, indices: np.ndarray, rbf_maps: dict, alpha=RIDGE_ALPHA, rls_lambda=RLS_LAMBDA):
...
def main():
    print(f"\n========== TASK: {TASK.upper()} (GES RBF) ==========")
    causal_path = os.path.join(PREFIX, f"{TASK}_normal_ges{ABL}.npz")
    ...
    fine_coeffs, indices, rbf_maps = fit_normal_coeffs(normal_data, normal_matrix)
    err_normal, norm_agg_normal = compute_online_errors(normal_data_full, fine_coeffs, normal_matrix, indices, rbf_maps)
    ...
    err_baseline, norm_agg_baseline = compute_windowed_errors(
        normal_data, window_len, fine_coeffs, normal_matrix, indices, rbf_maps)
    ...
    err_eval, _ = compute_windowed_errors(
        held_out, window_len, fine_coeffs, normal_matrix, indices, rbf_maps)
    ...
    for attack_name, attack_data in zip(attack_names, attack_data_list):
        ...
        err_attack, norm_agg_attack = compute_online_errors(
            attack_data, fine_coeffs, normal_matrix, indices, rbf_maps)
