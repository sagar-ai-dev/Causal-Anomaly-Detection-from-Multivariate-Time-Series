...
This chapter states the formalisms the detector is built on, and only those. It defines the structural causal model used to describe a multivariate time series, lists the assumptions under which the structure of such a model can be recovered from observation alone, and sets out the three discovery algorithms the evaluation compares. How those pieces are wired into a detector is the subject of Chapter~\ref{ch:methodology}; what they achieve on data is the subject of Chapter~\ref{ch:results}. The formalism is stated on its own terms and depends on neither. Where one of its assumptions is contradicted by a system this thesis actually runs on, though, the contradiction is recorded in this chapter and not left to a later one, because an assumption that fails on the evaluation data is a fact about this study and not a remark about the literature.
...
\emph{Faithfulness} is the converse direction: every conditional independence holding in the distribution is entailed by the graph. It rules out cancellation, where two causal paths of opposite sign sum to zero and a genuine dependence is invisible to any test. Faithfulness cannot be checked on data. It fails on a set of parameter values of measure zero, which sounds reassuring until one notices that engineered systems are built to cancel, and a control loop holding a variable at setpoint is a cancellation by design. The Tennessee Eastman process is that case exactly. Its 52 variables carry the plant's process measurements together with the manipulated variables through which the control loops act on the valves, and two loops close around the reactor, so a regulatory controller doing its job absorbs a disturbance by moving a valve and holds the measurement near setpoint. The dependence between the two is then partly cancelled in the record. Faithfulness is not a background technicality on that domain. It is contradicted by the plant's design, and the contradiction is present in the data before any algorithm reads it.

\emph{Causal sufficiency} requires every common cause of two measured variables to be measured. A latent driver violates it, and the symptom is a spurious edge between the two variables it drives. This is the assumption most often false in practice, since instrumenting a plant or a patient exhaustively is not an option. This pipeline then breaks it a second time, deliberately. Computational limits cap GES and CAM-UV at 35 variables on the robot, chosen by a label-blind variance filter, where PCMCI reads all 244. Any common cause that links two retained variables through a discarded one is latent by construction, and it is the cap and not the instrumentation that made it so. The robot's LED attack is the sharp case, since the filter excludes all 69 LED channels: the two algorithms scoring that fault hold no channel of the subsystem the fault was injected into. Chapter~\ref{ch:results} reports what they return anyway.

\emph{Causal stationarity} requires the mechanisms $f_j$ and the lag structure to be constant over the window searched. It is what makes a finite sample informative about a repeating graph. A detector built on drift is, in a sense, an instrument for finding where this assumption stops holding; discovery, however, needs it to hold on the training window. The pipeline takes an explicit position on it rather than an implicit one. Its recursive least squares filter carries a forgetting factor $\lambda$, and $\lambda = 1$ asserts stationarity over the whole evaluation stream while any value below one relaxes it. Robotics runs at $0.995$ and the other two domains at $1.0$, so the assumption is held on two systems and loosened on the one whose operating point moves. Chapter~\ref{ch:methodology} gives the update those values enter.
...
\paragraph{Drawbacks.} Everything rests on the model class being right, and the method does not report when it is not. Kernel independence testing is expensive in the number of observations and needs a substantial sample before it has power, which makes the method the most sample-hungry of the three. One of the three domains here does not meet that requirement. The plant supplies 0.67 samples per variable at the protocol's subsampling rate, and the synthetic ablation of Appendix~\ref{app:confounders} recovers a planted confounder at no significance level tested when it is given 200 samples. A sparse latent set on that domain measures absent power, then, and not absent confounding. The output is not a single connected structure but a mixture of oriented pairs, undecided pairs and latent-cause pairs, and a downstream consumer has to be told what to do with each. There is no lag, as with GES. And a latent-cause pair, correct as a finding, is precisely the thing a regression on measured variables cannot represent.
