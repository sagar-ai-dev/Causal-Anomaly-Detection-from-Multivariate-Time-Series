...
# Output prefix for the current run, set by generate_plots_and_reports.
_OUTPUT_PREFIX = ""
...
def plot_feature_importance_subplots(
    ...
    for i, norm_agg_attack in enumerate(norm_agg_list):
        ...
        csv_path = os.path.join(os.path.dirname(output_filename),
                                f"{_OUTPUT_PREFIX}explainability_top_features.csv")
...
def generate_plots_and_reports(
    ...
    global _OUTPUT_PREFIX
    _OUTPUT_PREFIX = output_prefix
    safe_variant_name = model_variant.lower().replace(" / ", "_").replace(" ", "_")
    output_plot_path = os.path.join(
        output_dir, f"{output_prefix}{safe_variant_name}_top_anomalous_variables.png")
