"""
==============================================================
 Causal Discovery & Anomaly Detection (GES RBF - Robotics)
==============================================================
Author: (Sagar)
"""
...
# Thesis-Verified Hyperparameters for RBF Robotics
RBF_GAMMA = 0.001
RBF_N_COMPONENTS = 100
RBF_RANDOM_STATE = 42
SUBSAMPLE_EXPECTED = 10
RIDGE_ALPHA = 1.0  # Relaxed alpha for RBF stability
...
def main():
    ...
    err_baseline, _ = compute_windowed_errors(normal_data, window_len, fine_coeffs, normal_matrix, indices, rbf_maps)
    baseline_len = next(iter(err_baseline.values())).shape[0]
    held_out = normal_data_full[len(normal_data):]
    err_eval, _ = compute_windowed_errors(held_out, window_len, fine_coeffs, normal_matrix, indices, rbf_maps)
    ...
    for attack_name, df_attack in zip(attack_names, attack_dfs):
        ...
        err_attack, norm_agg_attack = compute_online_errors(attack_data, fine_coeffs, normal_matrix, indices, rbf_maps)
