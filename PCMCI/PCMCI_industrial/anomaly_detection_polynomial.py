...
def fit_normal_poly_coeffs(normal_data: np.ndarray, causal_matrix: np.ndarray,
    ...
    for var in np.unique(indices[1, :]):
        ...
        # Keep the fitted intercept as coefficient 0 (a bias column is prepended online
        # to match). Storing only coef_ while fitting WITH an intercept left the online
        # baseline systematically offset, so RLS spent its adaptation chasing a constant.
...
def compute_online_poly_errors(data: np.ndarray, fine_coeffs: dict,
    ...
    for var in unique_vars:
        ...
        X_poly = np.column_stack([np.ones(len(X_poly)), X_poly])  # bias col matches stored intercept
...
def main():
    ...
    fine_coeffs, indices, poly_maps = fit_normal_poly_coeffs(normal_train, normal_matrix)
    err_baseline, norm_agg_baseline = compute_online_poly_errors(normal_train, fine_coeffs, normal_matrix, indices, poly_maps)
    ...
    err_normal_eval, _ = compute_online_poly_errors(normal_test, fine_coeffs, normal_matrix, indices, poly_maps)
