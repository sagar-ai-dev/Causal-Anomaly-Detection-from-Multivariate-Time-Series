...
def compute_windowed_rbf_errors(data: np.ndarray, window_len: int, fine_coeffs: dict,
                                 causal_matrix: np.ndarray, indices: np.ndarray,
                                 scalers: dict, rbf_maps: dict):
    """
    Run compute_online_rbf_errors over successive, non-overlapping windows of
    ...
    for w in range(n_windows):
        ...
        err_w, norm_agg_w = compute_online_rbf_errors(chunk, fine_coeffs, causal_matrix, indices, scalers, rbf_maps)
...
def main():
    ...
    err_normal_baseline, norm_agg_baseline = compute_windowed_rbf_errors(
        normal_data, window_len, fine_coeffs, normal_matrix, indices, scalers, rbf_maps
    )
    ...
    err_normal_eval, norm_agg_eval = compute_windowed_rbf_errors(
        held_out_data, window_len, fine_coeffs, normal_matrix, indices, scalers, rbf_maps
    )
