...
def fit_normal_poly_coeffs(normal_data: np.ndarray, causal_matrix: np.ndarray,
    ...
    for var in np.unique(indices[1, :]):
        ...
        # Keep the fitted intercept as coefficient 0 (a bias column is prepended to the
        # online design matrix to match). Previously only model.coef_ was stored while the
        # model was fitted WITH an intercept, so the online baseline was systematically
        # offset from the data and RLS spent its adaptation chasing that constant error.
...
def compute_online_poly_errors(data: np.ndarray, fine_coeffs: dict,
                                ...
                                poly_maps: dict, alpha: float = None, rls_lambda: float = None):
    """
    Recompute polynomial coefficients online sample-by-sample via RLS.

    alpha/rls_lambda default to the module-level RIDGE_ALPHA/RLS_LAMBDA read at
    CALL time (not definition time), so tuning sweeps that reassign the module
    globals actually take effect.
    """
    if alpha is None:
        alpha = RIDGE_ALPHA
    if rls_lambda is None:
    ...
    for var in unique_vars:
        ...
        # Bias column matching the intercept stored as fine_coeffs[var][0].
...
def compute_windowed_poly_errors(data: np.ndarray, window_len: int, fine_coeffs: dict,
                                  causal_matrix: np.ndarray, indices: np.ndarray, poly_maps: dict):
    """
    Run compute_online_poly_errors over successive, non-overlapping windows of
    `data`, each reinitialized fresh from fine_coeffs -- matching exactly how
    each attack file is scored -- instead of one long continuous run that lets
    the online coefficients drift arbitrarily far from the offline baseline
    over many thousands of steps regardless of whether anything anomalous is
    happening. Any leftover rows shorter than window_len are dropped.
    """
    ...
    for w in range(n_windows):
        ...
        err_w, norm_agg_w = compute_online_poly_errors(chunk, fine_coeffs, causal_matrix, indices, poly_maps)
...
def score_anomaly_timeline(err_baseline, err_eval, baseline_len):
    """
    Score err_eval (a fresh, single online-monitoring run) against a
    per-variable threshold derived from err_baseline (a fresh, windowed
    normal-baseline run of the same kind).
    """
...
def main():
    ...
    err_normal_baseline, norm_agg_baseline = compute_windowed_poly_errors(
    ...
    err_normal_eval, norm_agg_eval = compute_windowed_poly_errors(
        held_out_data, window_len, fine_coeffs, normal_matrix, indices, poly_maps
    )
