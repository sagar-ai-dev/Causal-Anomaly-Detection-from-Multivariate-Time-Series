"""
The XAI-course lab script with one change: Ridge in place of unregularised OLS.

Run verbatim, the lab script does not reproduce its own published result on this data -- it
attributes every attack class to laser range channels. The cause is numerical.

Several laser targets are regressed on the X *and* Y components of the same beam, which are
near-collinear, so the design matrix is numerically singular at double precision:

    cond(X)   LaserF15X      3.7e+09     parents LElbowYaw, LaserR07X, LaserR07Y
    cond(X)   LaserF05X      4.4e+09     parents LShoulderPitch, LaserR09X, LaserR09Y
    cond(X)   TabletTouchX   3.2e+02
    cond(X)   KneePicthTemp  3.8e+02
    cond(X)   WheelFRTemp    3.6e+02

`np.linalg.lstsq(..., rcond=None)` returns a minimum-norm solution for those, with
coefficients around 1e11 against ~1e4 for well-conditioned targets. Since attribution ranks
by the L2 norm of the coefficient drift, the blow-up dominates and buries the real signal.

Replacing the two `lstsq` calls with `Ridge(alpha=10.0)` -- the only change here -- restores
the published result exactly:

    WheelsControl -> WheelFRTemp, WheelBTemp, KneePicthTemp
    JointControl  -> KneePicthTemp, WheelFRTemp, WheelBTemp
    LedsControl   -> TabletTouchX, TabletTouchY, WheelFRTemp

It also removes an artefact in the metrics. Unregularised, the blow-up inflates the detection
threshold so far that nothing in the fault-free tail can exceed it, giving precision 1.0000
and fp=0. With Ridge: precision 0.8667, fp=16, F1 0.8927 (from 0.9585).

The thesis pipeline is immune to this by construction -- Ridge for the offline fit, and RLS
initialised at those coefficients with P0 = I/alpha, which is implicitly regularised. That is
why it reproduces the lab plot where the unregularised script cannot.

Not part of the pipeline; nothing imports it.
"""
...
def fit_normal_coeffs(normal_data: np.ndarray, causal_matrix: np.ndarray):
    ...
    for var in np.unique(indices[1, :]):
        ...
        _m = Ridge(alpha=10.0, fit_intercept=True).fit(X, y)
        coeffs = np.concatenate([[float(_m.intercept_)], np.ravel(_m.coef_)])
...
def main():
    ...
    causal_path = os.path.join(PREFIX, f"{TASK}_normal_LABRIDGE.npz")
    ...
    for path, df_attack in zip(attack_paths, attack_dfs):
        ...
        np.save(f"_ridgedump_{attack_name}.npy", norm_agg_attack)
        ...
        print("  LABRIDGE_ATTR " + attack_name + ": " + ", ".join(f"{k}" for k, _ in top3))
    ...
    print("\n========== LAB METRICS ==========")
    print(f"LABRIDGE Precision: {precision:.4f}")
    print(f"LABRIDGE Recall:    {recall:.4f}")
    print(f"LABRIDGE F1 Score:  {f1:.4f}")
    print(f"LABRIDGE Accuracy:  {accuracy:.4f}")
    print(f"LABRIDGE AUC-ROC:   {auc_roc:.4f}")
    print(f"LABRIDGE AUC-PR:    {auc_pr:.4f}")
    print(f"LABRIDGE tp={np.sum(tpos)} fp={np.sum(fpos)} fn={np.sum(fneg)}")
    print("=================================\n")
